function preload() {
  ...
  restartBtn = loadImage("data/playagain.png");
  jumpBtn = loadImage("data/jump.png");
  guide = loadImage("data/guide.png");
...
function setup() {
  ...
  if(windowWidth*8/6 < windowHeight){
  var myCanvas = createCanvas(windowWidth, windowWidth*8/6);}
  else{
  var myCanvas = createCanvas(windowHeight*6/8*0.78, windowHeight*0.78);
  }
  ...
  jikiSpeed = (jikiSpeedDefault + jikiSpeedSlow) / 2;
...
function setupSprites() {
  ...
  jumpBtn.resize(width*0.8, jumpBtn.height*width*0.8/jumpBtn.width);
  guide.resize(width*0.9, guide.height*width*0.9/guide.width);
